...
display_menu() {
    ...
    echo -e " 8. Setup Auto-Monitoring (Backhaul Services)"
    echo -e " 9. Check Monitoring Log"
    echo -e "10. Check Backhaul Service Status"
    echo -e " 0. Exit"
    echo
    echo "-------------------------------"
}
...
read_option() {
    ...
    case $choice in
        ...
        6) update_script ;;
        7) remove_core ;;
        8) setup_backhaul_monitoring ;;
        9) check_monitoring_log ;;
        10) check_backhaul_services ;;
        0) exit 0 ;;
        *) echo -e "${RED} Invalid option!${NC}" && sleep 1 ;;
    esac
}
...
setup_backhaul_monitoring() {
    echo "🔍 شناسایی سرویس‌های backhaul..."
    mapfile -t services < <(systemctl list-units --type=service --all | grep -oP '^backhaul[^\s]*')

    if [[ ${#services[@]} -eq 0 ]]; then
        echo "❌ هیچ سرویسی با پیشوند backhaul پیدا نشد."
        return
    fi
    ...
    echo
    read -p "🔘 شماره سرویس‌هایی که می‌خوای مانیتور بشن (با کاما جدا کن مثل 1,3): " selection

    IFS=',' read -ra indices <<< "$selection"
    selected_services=()
    for index in "${indices[@]}"; do
        i=$((index - 1))
        if [[ $i -ge 0 && $i -lt ${#services[@]} ]]; then
    ...
    echo
    read -p "🌐 لطفاً IP سرور مقابل (برای ping و بررسی تونل) را وارد کنید: " peer_ip
    echo "$peer_ip" > /etc/backhaul-peer-ip


    if [[ ${#selected_services[@]} -eq 0 ]]; then
        echo "⚠️ هیچ سرویسی انتخاب نشد."
        return
    fi

    echo "🛠 در حال ساخت اسکریپت مانیتورینگ..."

    cat <<EOF > /root/backhaul-monitor.sh
#!/bin/bash
...
for service in "\${selected_services[@]}"; do
    ...
    if [[ "\$status" != "active" ]]; then
        echo "\$(date) - \$service is \$status. Restarting..." >> /var/log/backhaul-monitor.log
        systemctl restart "\$service"
        sleep 1
        if [[ "\$(systemctl is-active "\$service")" != "active" ]]; then
            echo "\$(date) - Failed to restart \$service!" >> /var/log/backhaul-monitor.log
        else
            echo "\$(date) - \$service restarted successfully." >> /var/log/backhaul-monitor.log
        fi
    fi
done
EOF

    chmod +x /root/backhaul-monitor.sh

    echo "⚙️ ایجاد سرویس systemd برای مانیتورینگ..."

    cat <<EOF > /etc/systemd/system/backhaul-monitor.service
[Unit]
Description=Backhaul Auto Monitoring
After=network.target

[Service]
ExecStart=/bin/bash /root/backhaul-monitor.sh
Restart=always
RestartSec=30
...
EOF
    ...
    echo "✅ مانیتورینگ فعال شد و هر ۳۰ ثانیه سرویس‌ها رو چک می‌کنه."
}
...
check_monitoring_log() {
    echo "📄 آخرین لاگ‌های مانیتورینگ:"
    if [[ -f /var/log/backhaul-monitor.log ]]; then
        tail -n 30 /var/log/backhaul-monitor.log
    else
        echo "⛔ لاگ مانیتورینگ پیدا نشد!"
    fi
    echo
    read -p "برای ادامه Enter بزنید..." enter
}

check_backhaul_services() {
